...
exports.hubtool = onRequest({
  ...
  const { hubSlug, slot, messages, generateMode } = req.body;
  if (!hubSlug || !Array.isArray(messages)) {
    res.status(400).json({ error: "hubSlug and messages required" });
    return;
  }
  ...
  try {
    ...
    const toolData = hub[slotKey] || {};
    const systemPrompt = toolData.systemPrompt || hub.systemPrompt || "You are a helpful AI assistant.";
    const toolName     = toolData.toolName     || hub.toolName     || "AI Tool";
    const model        = hub.toolModel || hub.model || "claude-haiku-4-5-20251001";

    const anthropic = new Anthropic({ apiKey });

    if (generateMode) {
      const triggerMsg = `Generate my full ${toolName} now. Write the complete report from scratch based on everything I shared. Do not reference previous messages or say I already have it — produce the full output now.`;
      const generateSys = `You are a report generator. The intake conversation is complete. Your ONLY job is to write the full, complete ${toolName} report from scratch based on everything shared in the conversation. Write it now — do not ask questions, do not reference previous messages, do not explain what you are doing. Just produce the complete report.`;

      const response = await anthropic.messages.create({
        model,
        max_tokens: 2000,
        system: generateSys,
        messages: [...messages, { role: "user", content: triggerMsg }]
      });
      res.status(200).json(response);

    } else {
      const fullSystem = systemPrompt +
        "\n\n---\nIMPORTANT SIGNAL INSTRUCTIONS: When you have asked enough questions and have all the information you need, do NOT generate the final output or report in this response. Instead, write a single short closing sentence (e.g. \"I have everything I need to build your results!\") and then add [READY_TO_GENERATE] on its own line. The actual output will be generated separately when the user clicks the button — do not include it here. Never mention or explain [READY_TO_GENERATE] to the user.";
    ...
    console.error("hubTool error:", err.message);
    if (!res.headersSent) {
      res.status(500).json({ error: err.message });
    } else {
      res.end();
